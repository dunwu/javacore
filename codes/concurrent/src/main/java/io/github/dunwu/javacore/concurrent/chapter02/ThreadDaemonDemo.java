 ...
 * 守护线程示例
 * <p>
 * 什么是守护线程 守护线程（Daemon Thread）是在后台执行并且不会阻止 JVM 终止的线程。 与守护线程（Daemon Thread）相反的，叫用户线程（User
 * Thread），也就是非守护线程。
 * <p>
 ...
 * <p>
 * 注意点： 正在运行的用户线程无法设置为守护线程，所以 thread.setDaemon(true)必须在 thread.start()之前设置，否则会抛出
 * llegalThreadStateException 异常； 一个守护线程创建的子线程依然是守护线程。 不要认为所有的应用都可以分配给 Daemon
 * 来进行服务，比如读写操作或者计算逻辑。
 *
 * @author Zhang Peng
 * @date 2018/1/18
