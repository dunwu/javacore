package io.github.dunwu.javacore.basics;
 ...
 * 示例代码的临时文件工具。
 * <p>
 * 本模块的序列化示例（{@code serial}、{@code serialize} 包）会生成 {@code .dat} 数据文件。
 ...
 *
 * <p>
 * <b>包位置</b>：本类刻意放在模块专属包 {@code io.github.dunwu.javacore.basics}，而不是模块根包
 * {@code io.github.dunwu.javacore}。{@code javacore-io} 模块也有一个 {@code DemoFiles}，同处根包会让
 * 两者的 FQCN 完全一致；当前 18 个模块互不依赖，两份还不会共存于同一 classpath，但只要有任何模块
 * 开始依赖 {@code javacore-basics}，JVM 就会按 classpath 顺序静默选中其中一份。请勿移回根包。
 *
...
public final class DemoFiles {
     ...
     * <p>
     * 例：{@code temp("serialize01.dat")} → {@code target/serialize01.dat}
     */
