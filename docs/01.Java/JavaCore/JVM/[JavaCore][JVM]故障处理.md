...
# 用 MAT 分析引用链，找到 GC Root 持有大对象的泄漏点
```

### 场景三：死锁检测

```bash
jstack -l <pid> | grep -B5 -A20 "Found one Java-level deadlock"
```

## 最佳实践

1. **开启 GC 日志和 HeapDump**：事前配置优于事后补救。
2. **定期监控 JVM 指标**：堆使用率、GC 频率、线程数。
3. **建立基线指标**：正常状态下的指标作为参照，异常时对比。
4. **保留现场**：OOM 时自动 dump，死锁时 jstack，不要先重启。

## 常见问题

### Q1：如何判断是 CPU 问题还是内存问题？

CPU 问题通常表现为线程死循环或计算密集；内存问题表现为 GC 频繁、OOM。通过 `top` 看 CPU，`jstat -gcutil` 看 GC 频率。

### Q2：频繁 Full GC 如何排查？

分析 GC 日志确认频率和耗时，dump 堆快照查看老年代对象分布，检查是否有大对象直接进入老年代或内存泄漏。

### Q3：如何排查线程死锁？

使用 `jstack -l <pid>` 可以直接检测死锁并输出互相等待的线程和锁信息。
