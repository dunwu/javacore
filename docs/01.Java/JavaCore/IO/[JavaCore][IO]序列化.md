...
序列化是将对象状态转换为可存储或可传输的字节流的过程，反序列化则是将字节流恢复为对象。Java 序列化广泛应用于 RPC 通信、分布式缓存、会话持久化等场景。除了 JDK 原生序列化，业界还有 Protobuf、Thrift、Hessian、JSON 等多种序列化方案。
...
User restored = mapper.readValue(cached, User.class);
```

### 场景二：RPC 通信序列化

微服务间的 RPC 调用使用 Protobuf 进行高效序列化：
...
多节点部署时，通过序列化将 Session 数据存入 Redis：

```java
// Session 对象必须实现 Serializable
...
1. **优先使用 JSON 或 Protobuf**：JDK 原生序列化性能差、体积大、安全性低，生产环境不推荐。
2. **始终声明 `serialVersionUID`**：防止类结构变化导致反序列化失败。
3. **敏感字段用 `transient` 修饰**：密码、密钥等敏感信息不应被序列化。
4. **使用 `Externalizable` 自定义序列化逻辑**：当需要精细控制序列化过程时。
5. **注意序列化安全性**：反序列化可能执行任意代码，对不可信数据应先校验签名或使用 `ObjectInputFilter`。
6. **选择跨语言方案**：如果系统涉及多种语言，Protobuf/Thrift 是更好的选择。

## 常见问题

### Q1：JDK 序列化和 JSON 序列化有什么区别？
...
### Q2：为什么 `serialVersionUID` 很重要？

`serialVersionUID` 是序列化版本号。如果不显式声明，JVM 会根据类结构自动生成，一旦类结构发生变化（如增加字段），自动生成的 UID 会改变，导致反序列化失败抛出 `InvalidClassException`。

### Q3：`transient` 关键字的作用是什么？

`transient` 修饰的字段不参与序列化。常用于密码、数据库连接、临时缓存等不需要持久化的字段。注意 `static` 字段也不会被序列化。
