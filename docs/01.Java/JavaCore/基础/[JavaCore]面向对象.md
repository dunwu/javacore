...
tags:
  ...
  - JavaCore
  - 面向对象
permalink: /pages/aab0a943/
---

# Java 面向对象

> 在[Java 基本数据类型]([JavaCore]数据类型.md) 中我们了解 Java 中支持的基本数据类型（值类型）。本文开始讲解 Java 中重要的引用类型——类。

## 面向对象

每种编程语言，都有自己的操纵内存中元素的方式。

Java 中提供了[基本数据类型](https://dunwu.github.io/waterdrop/pages/cba76603/)，但这还不能满足编写程序时，需要抽象更加复杂数据类型的需要。因此，Java 中，允许开发者通过类（类的机制下面会讲到）创建自定义类型。
...
- 几乎所有 Java 对象初始化时，都要使用 `new` 创建对象（[基本数据类型](https://dunwu.github.io/waterdrop/pages/cba76603/)、String、枚举特殊处理），对象存储在堆中。
...
public abstract class Order {
    ...
    public abstract void process();  // 多态：不同订单有不同处理逻辑
}
...
public class AlipayStrategy implements PaymentStrategy {
    @Override
    public boolean pay(BigDecimal amount) { /* 支付宝支付 */ }
}

public class WechatPayStrategy implements PaymentStrategy {
    @Override
    public boolean pay(BigDecimal amount) { /* 微信支付 */ }
}
...
public class PaymentService {
    ...
    public void executePayment(BigDecimal amount) { strategy.pay(amount); }
}
```

### 场景三：模板方法模式

利用抽象类定义算法骨架，子类实现具体步骤：
...
public abstract class DataExporter {
    ...
    // 公共步骤 - 复用实现
    private void formatData() { /* 格式化逻辑 */ }
    private void writeOutput() { /* 输出逻辑 */ }
}
```

## 最佳实践

1. **组合优于继承**：继承耦合度高，优先考虑组合方式实现代码复用。
2. **接口隔离原则**：接口应该小而专一，避免庞大臃肿的接口。
3. **优先使用抽象类或接口声明变量**：`List<String> list = new ArrayList<>()` 优于 `ArrayList<String> list = new ArrayList<>()`。
4. **封装优先**：成员变量默认声明为 `private`，通过 getter/setter 控制访问。
5. **避免过深的继承层次**：继承层次超过 3 层时应考虑使用组合或设计模式重构。
6. **正确使用访问修饰符**：内部实现用 `private`，子类可访问的用 `protected`，对外 API 用 `public`。
7. **覆写 equals 必须同时覆写 hashCode**：确保在 HashMap、HashSet 等散列容器中行为正确。
...
| --- | --- | --- |
| 实例化 | 不能 | 不能 |
| 方法实现 | 可以有具体方法 | JDK8 前只能有抽象方法 |
| 构造方法 | 有 | 没有 |
| 继承 | 单继承 | 多实现 |
| 成员变量 | 可以有各种修饰 | 默认 public static final |
| 设计目的 | 代码复用（is-a） | 行为规范（can-do） |

### Q2：Java 为什么不支持多重继承？

Java 设计者认为多重继承会导致“菱形问题”（两个父类有相同方法时子类不确定调用哪个）。Java 通过“单继承 + 多接口实现”来避免这个问题，同时接口的 default 方法（JDK8）提供了一定程度的代码复用能力。

### Q3：内部类和静态内部类有什么区别？

- **内部类（非静态）**：持有外部类的引用，可以访问外部类的所有成员，但不能包含静态成员。
- **静态内部类**：不持有外部类引用，只能访问外部类的静态成员，可以包含静态成员。性能更好，推荐优先使用。
