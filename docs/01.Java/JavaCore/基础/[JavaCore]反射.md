...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2019/03/e7386dacbce04b4c8840cc6d755884c0.png)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2026/02/913f2977ba914b58b1c37596966f3ff5.webp)
...
![](https://raw.githubusercontent.com/dunwu/images/master/archive/2019/03/c80619b22c024745b60814b553c9713f.png)
...
1. **避免在热点代码中使用反射**：反射操作的性能比直接调用低数倍到数十倍，频繁调用时应缓存结果或使用字节码生成。
2. **缓存 Method/Field 对象**：`getMethod` 和 `getDeclaredField` 每次都会返回新对象，应缓存到 Map 中复用。
3. **避免使用 `getMethods()` 遍历**：该方法返回包括继承方法在内的所有方法，在热路径中会产生不必要的堆分配。
4. **注意安全性**：反射可以访问私有成员，可能破坏封装性。在模块系统（JDK9+）中，反射访问还受到模块边界限制。
5. **优先使用 JDK 动态代理**：如果目标类实现了接口，优先使用 JDK 动态代理，代码更简洁且维护性高。
6. **CGLIB 用于无接口场景**：如果目标类没有实现接口，使用 CGLIB 基于继承的代理方式。
...
反射性能低的原因有三：
1. 动态解析类型，无法 JIT 优化和内联。
2. `Method.invoke` 是变长参数方法，需要创建 Object 数组。
3. 基本类型参数需要自动装箱/拆箱。

但在调用次数超过阈值（默认 15 次）后，JVM 会生成字节码（Inflation），性能会大幅提升。
...
由于类型擦除，运行时无法直接获取泛型参数。但可以通过反射获取父类或字段的泛型信息：
