...
* 轮询锁和定时锁时由 tryLock 方法实现的。
* synchronized vs. ReentrantLock
* ReentrantLock 的优点：可定时、可轮询、可中断、公平队列。
* 读写锁的应用场景；一个资源可以被多个读操作访问，或者被一个写操作访问，但两者不能同时进行。
...
* 锁的劣势
* 使用锁在挂起和恢复线程等过程中存在着很大的开销，并且通常存在着较长时间的中断。
* 当一个线程在等待锁时，它不能做任何其他事。
* CAS
  * CAS 包含了 3 个操作数——需要读写的内存位置 V、进行比较的值 A 和将要写入的新值 B。当且仅当 V 的值等于 A 时，CAS 才会通过原子方式用新值 B 来更新 V 的值，否则不会执行任何操作。无论位置 V 的值是否等于 A，都将返回 V 原有的值。
  * CAS 的含义是：我认为 V 的值应该为 A，如果是，那么将 V 的值更新为 B，否则不修改并告诉 V 的值实际为多少。
  * CAS 的主要缺点是，它将使调用者处理竞争问题，而在锁中能自动处理竞争问题。
* 原子变量类
  * 原子变量比锁的粒度更细，量级更轻，并且对于在多处理器系统上实现高性能的并发代码来说非常关键。
* 非阻塞算法
  * 一个线程的失败或挂起不会导致其他线程也失败或挂起，那么这种算法就称为非阻塞算法。
