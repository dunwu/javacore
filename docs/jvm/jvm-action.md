...
- [一、JVM 调优概述](#一jvm-调优概述)
  - [调优原则](#调优原则)
  - [GC 优化的过程](#gc-优化的过程)
- [二、典型配置](#二典型配置)
  - [堆大小设置](#堆大小设置)
  - [回收器选择](#回收器选择)
- [三、分析 GC 日志](#三分析-gc-日志)
  - [获取 GC 日志](#获取-gc-日志)
  - [如何分析 GC 日志](#如何分析-gc-日志)
- [附录 - 常用 HotSpot VM 参数](#附录---常用-hotspot-vm-参数)
  - [JVM 内存配置](#jvm-内存配置)
  - [GC 类型配置](#gc-类型配置)
  - [垃圾回收器通用参数](#垃圾回收器通用参数)
  - [JMX](#jmx)
...
- `延迟（latency）` - 其度量标准是缩短由于垃圾啊收集引起的停顿时间或者完全消除因垃圾收集所引起的停顿，避免应用运行时发生抖动。
- `内存占用（footprint）` - 垃圾收集器流畅运行所需要的内存数量。
