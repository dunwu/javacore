...
CPU 使用率过高，往往是由于程序逻辑问题导致的，常见场景如：死循环，无限递归、频繁 GC、线程上下文切换过多。
...
一般先使用 `jstat` 来查看分代变化情况，比如 minor gc 或 full gc 次数是不是太频繁。
...
OOM 定位可以参考：[JVM 内存管理之OutOfMemoryError](jvm-memory.md#三OutOfMemoryError)

### Minor GC 过频

Minor GC 频繁一般是短周期小对象较多，先考虑是不是 Eden 区/新生代设置的太小了，看能否通过调整 `-Xmn、-XX:SurvivorRatio` 等参数设置来解决问题。如果参数正常，但是 Minor GC 频率还是太高，就需要使用 Jmap 和 MAT 对 dump 文件进行进一步诊断了。
